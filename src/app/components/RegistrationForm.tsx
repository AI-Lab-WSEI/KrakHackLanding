      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {formTypes.map((type, index) => {
            ...
            const isParticipant = type.title === 'Uczestnik';
            const isClosed = isParticipant && new Date() >= new Date('2026-03-27T18:00:00');
            ...
            return (
              <motion.div
                ...
                className={`
                  relative group block p-8 bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 
                  transition-all duration-300 
                  ${isClosed ? 'opacity-50 grayscale cursor-not-allowed' : 'hover:border-gray-600 hover:transform hover:scale-105 cursor-pointer'}
                `}
                onClick={() => {
                  if (!isClosed) {
              ...
              >
                <div className="text-center">
                  <div className={`
                    ...
                    ${!isClosed && 'group-hover:shadow-lg'} transition-all duration-300
                  `}>
                  ...
                  <p className="text-gray-300 mb-6 leading-relaxed">
                    {isClosed ? 'Rekrutacja uczestników została zakończona o 18:00.' : type.description}
                  </p>
                  
                  <div className={`
                    inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r ${type.color} 
                    ${isClosed ? 'opacity-50' : `${type.hoverColor} group-hover:shadow-lg`} text-white rounded-lg transition-all duration-300 
                    font-semibold
                  `}>
                    <span>{isClosed ? 'Rejestracja zamknięta' : 'Wypełnij formularz'}</span>
                    {!isClosed && <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />}
                  </div>
                </div>
              </motion.div>
            );
